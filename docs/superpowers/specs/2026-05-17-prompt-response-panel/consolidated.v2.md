...
**Supersedes:** `consolidated.v1.md` (kept for diff reference)

## Changes from v1 (addressing the reviewer)

| # | Reviewer finding | v1 → v2 change |
|---|------------------|----------------|
| 1 | `action_parsed` lacks attempt | **Adopted in full.** Added `attempt` to `action_parsed`; new `turn/action_parse_failed` event with `{turn, attempt, error}`; new `_on_action_parse_failed` hook |
| 2 | `used_response_format` not derivable from attempt | **Adopted.** Field semantics rewritten; new test `test_on_llm_response_attempt_1_can_have_used_response_format_false` |
| 3 | Request metadata vs prompt | **Partially adopted.** Added separate `system` field (spec lied — code passes `system=_SYSTEM_PROMPT` separately). **Pushed back** on `temperature`/`max_tokens`/`messages` rename: gold-plating for v1 (see non-goals) |
| 4 | Header status not backed by state | **Adopted, simplified.** `getTurnStatus(turnState)` helper as single source of truth; no redundant `status` field on state |
| 5 | Delta inside every attempt card is wasteful | **Adopted.** Panel body restructured: prompt section ONCE per turn, attempt cards show response + status only |
| 6 | Automated frontend safety test | **Adopted.** New `tests/dashboard/test_probe_detail_static.py` with pytest assertion |
| 7 | Size budget optimistic | **Acknowledged, push-back on fix.** Documented that worst case can exceed 180 KB; **rejected** hard truncation (defeats observability); soft-policy documented |
| 8 | "Colour-coded" wording | **Adopted.** Replaced with "warning/default styling (icon + border, not color-alone)" |
| 9 | Canonical static path | **Adopted.** All paths now use `src/earn_money/dashboard/templates/static/` |
| 10 | No copy buttons | **Adopted.** Added to non-goals |
| + | Reviewer's acceptance-criteria list | **Adopted.** New "Acceptance Criteria" section in `00-overview.md` |
...
4. **Header status comes from one helper.** A single `getTurnStatus(turnState)` function in `probe-detail.js` produces the badge text; both timeline and detail panel call it.
...
`system` is `_SYSTEM_PROMPT` (module constant, same value every call). `prompt` is the result of `_build_prompt()` for the turn. `attempt` is `1` on the first call, `2` on the retry. `used_response_format` is read from `self._last_used_response_format` which the helper sets from its own return value — NOT derived from attempt number.
...
Prompts ride the existing `/api/probe/stream` SSE channel as part of `turn/action_pending`, `turn/action_parsed`, and the new `turn/action_parse_failed` events. Server keeps no per-run prompt cache; once an event is sent, the data is the browser's responsibility.
...
state.turns = {
  1: {
    ...
    attempts: [
      { attempt, raw, model, used_response_format, ts, parseOutcome }
...
A single helper lives in `probe-detail.js`:

```js
function getTurnStatus(turnState) {
...
Both the timeline summary (left column) and the detail header (right column) call `getTurnStatus()`. The header text ("invalid_action → recovered", etc.) is built from this single source. No status enum is stored on `state.turns[N]` — always derived.

## Boundary discipline

- **Timeline renderer** owns left-column rendering. Reads `state.turns` only for "which turns exist + summary". Calls `getTurnStatus()` for badge text.
- **Detail panel renderer** owns right-column rendering. Reads `state.turns[selectedTurn]` only. Calls `getTurnStatus()` for header text.
- **Probe dispatcher** (`probe.js`) owns selection + tab state. Re-emits change events for both renderers.

The two renderers do not call each other. `getTurnStatus()` is the only shared logic.
...
Status comes from `attempts[i].parseOutcome`, which the dispatcher sets when `turn/action_parsed` (`"ok"`) or `turn/action_parse_failed` (`"failed"`) arrives — matched by `(turn, attempt)`, never inferred from event order.
...
| `src/earn_money/dashboard/templates/static/probe-detail.js` | ≤170 | Right-panel renderer: `getTurnStatus()` helper, header builder, tab switcher, delta-diff function, attempt-card builder. Reads from `state`, no DOM coupling to the timeline. Status helper is the single source of truth for both timeline and detail header text |

`probe-detail.js` is the most substantive new module. Key sub-pieces: the `getTurnStatus()` helper (~15 lines) and the delta-diff function (`splitPromptSections(prompt) → { sectionName: body }`, then compare two maps and keep only differing sections, ~30 lines).
...
- 1 server route file modified (two-line additions to `_STATIC_ROUTES`)
