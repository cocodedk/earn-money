...
- On `livePolling=false` OR unmount: stop polling, status → `closed`. `livePolling=false` is triggered by `isRunActive(status)` returning false — that's two categories: (a) **terminal statuses** `done` / `stopped` / `failed` (run is over, won't resume), and (b) **resumable status** `paused` (run will resume; buffer is preserved across the cycle per Phase 3 Task 5). Both close the polling loop, but only (a) is end-of-life; (b) re-enters SSE on resume.
