...
5. Add `FINDING_CREATED = "finding.created", "Finding created"` to `backend/apps/events/types.py` `EventType` enum (between `FINDING_STATUS_CHANGED` and any later entries). Add a test in `backend/apps/events/tests.py` that asserts `EventType.FINDING_CREATED.value == "finding.created"` and that `Event.log(type=EventType.FINDING_CREATED, ...)` round-trips. This is the SINGLE owner site for the enum addition — no other slice adds it; `rollback.md` revert path is WKP-A only.
6. /simplify round 1, commit. Commit subject: `feat(stubs): well_known_paths families + signatures + soft-404 + apps.py + FINDING_CREATED enum`.
