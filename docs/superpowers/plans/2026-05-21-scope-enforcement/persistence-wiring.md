...
* The resolved `Program` (`platform`, `slug`, `scope`, `roe`) is attached to the scan-run runtime context and passed into every runner/fetcher. Do not re-resolve from a mutable target URL inside the fetcher.
