...
4. Store/pass the resolved `Program` runtime context (`platform`, `slug`, `scope`, `roe`) into the runner payload used by slices E-G as serializable primitives; reconstruct dataclasses inside the worker. Do not make fetchers re-resolve the target.
