...
Reads discovered URLs from the latest katana `discovered_urls.jsonl` artifact,
filters those with GET parameters, passes them to sqli_tool for error-based
and boolean-based injection probing. Emits `sqli_candidate` signals.

Prerequisite: a recent katana run with output_count > 0.
"""
...
def _recent_katana_artifact(
    ...
    """Return artifact_dir of the most recent successful katana run, or None."""
...
def _load_discovered_urls(artifact_dir: str) -> list[str]:
    """Return GET-parameterized URLs from the katana discovered_urls.jsonl."""
...
def run_program(
    ...
    *,
    ...
    artifact_dir = paths.root / (
        f"recon/outputs/{platform}/{slug}/sqli/{now[:10]}/{run_id}"
    )
    ...
    try:
        ...
        if katana_artifact is None:
            return record_prereq_missing(
                ...
                run_id=run_id, now=now, artifact_dir=artifact_dir, tool="sqli",
        ...
        runs.start_run(
            conn, run_id=run_id, platform=platform, slug=slug, tool="sqli",
