...
class ConfirmedFindingTests(unittest.TestCase):
    def test_comment_reference_confirmed_high_confidence(self) -> None:
        ...
        v = classify_map_result(
            resolved=_resolved_ok(), map_outcome=_ok_map(),
            metadata=_metadata(), reference_type="comment",
        )
        assert v.finding_status == FindingStatus.CONFIRMED
    ...
    def test_fallback_reference_confirmed_medium_confidence(self) -> None:
        ...
        v = classify_map_result(
            resolved=_resolved_ok(), map_outcome=_ok_map(),
            metadata=_metadata(), reference_type="fallback",
        )
        assert v.finding_status == FindingStatus.CONFIRMED
...
class InlineDataUrlTests(unittest.TestCase):
    def test_inline_data_url_yields_candidate_no_fetch(self) -> None:
        ...
        v = classify_map_result(
            ...
            map_outcome=None, metadata=None, reference_type="comment",
        )
        assert v.finding_status == FindingStatus.CANDIDATE
...
class RejectedFindingTests(unittest.TestCase):
    def test_404_map_yields_rejected(self) -> None:
        ...
        assert v.finding_status == FindingStatus.REJECTED
        assert v.confidence == "low"
...
class InvariantTests(unittest.TestCase):
    def test_ok_resolved_without_map_outcome_raises(self) -> None:
        # The runner is contractually required to fetch when
        # resolved.kind == "ok". A None map_outcome is a wiring bug
        # that should surface loudly, not silently misclassify.
        with self.assertRaises(RuntimeError):
            classify_map_result(
                resolved=_resolved_ok(), map_outcome=None,
                metadata=None, reference_type="comment",
            )
