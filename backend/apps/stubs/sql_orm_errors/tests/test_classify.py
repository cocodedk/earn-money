...
def test_strong_signature_with_error_status_is_high() -> None:
    ...
    v = cls.classify(status=500, headers={}, body=body)
...
def test_strong_signature_with_ok_status_is_medium() -> None:
    ...
    v = cls.classify(status=200, headers={}, body=body)
    assert v is not None
...
def test_no_match_returns_none() -> None:
    assert cls.classify(status=200, headers={}, body=b"<html>OK</html>") is None
...
def test_rfc7807_problem_json_without_signature_is_none() -> None:
    ...
    assert cls.classify(status=500, headers={"content-type": "application/problem+json"}, body=body) is None


def test_server_header_alone_is_none() -> None:
    # "Server: nginx" alone doesn't trigger any signature.
    body = b"<h1>Forbidden</h1>"
    assert cls.classify(status=403, headers={"server": "nginx/1.21.0"}, body=body) is None
...
# --- Framework hints (when matcher exposes them) ---


def test_framework_hints_collected_from_body() -> None:
    body = b"django.db.utils.OperationalError: relation \"foo\" does not exist"
    v = cls.classify(status=500, headers={}, body=body)
    # The strongest sig here is generic_sql.unterminated_string? No — `relation "foo" does not exist`
    # only matches if we have a signature for it. None defined → matcher returns None
    # → verdict is None. This documents the matcher's behavior.
    # We DO match the django.db.utils path though? Not a signature. So this body
    # produces NO verdict in the current MVP.
    assert v is None  # spec coverage gap; tracked for future signature additions
