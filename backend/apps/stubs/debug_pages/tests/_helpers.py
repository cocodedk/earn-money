"""Shared test helpers for stub 1.10."""
...
def _mock_get(responses: dict[str, httpx.Response]):
    ...
    probe like `/_profiler/` doesn't get shadowed by the `/` baseline
    via `url.endswith("/")` matching every URL."""
...
def mocked_fetcher(
    ...
    *,
    ...
    with patch(
        "apps.stubs.debug_pages.fetcher.httpx.Client"
    ) as mock_client:
