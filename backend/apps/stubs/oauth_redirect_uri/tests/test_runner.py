...
@pytest.mark.django_db
def test_invalid_json_oidc_body_exits_cleanly(monkeypatch):
    monkeypatch.setenv("FIXTURE_OAUTH_REDIRECT_URI_URL", _BASE)
    monkeypatch.setenv("FIXTURE_OAUTH_CLIENT_SECRET", "fixture-value")
    scan_run, target_run = seed_target_run(host="oauth-redirect-uri-lab", stub_slug="2.14")
    bad_body = FetchOutcome(ok=True, status=200, body="not-json{{{",
                            content_type="text/html", final_url="", error=None)
    with patch.object(get_registry(), "find_for_host", return_value=_program()):
        with patch(f"{_MODULE}.fetch_for_discovery", return_value=bad_body):
            run(scan_run, target_run)
    assert not Finding.objects.filter(scan_run=scan_run, stub_slug="2.14").exists()


@pytest.mark.django_db
def test_none_probe_response_continues_loop(monkeypatch):
    monkeypatch.setenv("FIXTURE_OAUTH_REDIRECT_URI_URL", _BASE)
    monkeypatch.setenv("FIXTURE_OAUTH_CLIENT_SECRET", "fixture-value")
    scan_run, target_run = seed_target_run(host="oauth-redirect-uri-lab", stub_slug="2.14")
    with patch.object(get_registry(), "find_for_host", return_value=_program()):
...
@pytest.mark.django_db
def test_candidate_response_creates_medium_finding(monkeypatch):
    monkeypatch.setenv("FIXTURE_OAUTH_REDIRECT_URI_URL", _BASE)
    monkeypatch.setenv("FIXTURE_OAUTH_CLIENT_SECRET", "fixture-value")
    scan_run, target_run = seed_target_run(host="oauth-redirect-uri-lab", stub_slug="2.14")
    candidate_resp = MagicMock()
    candidate_resp.status_code = 200
    candidate_resp.headers = {}
    candidate_resp.text = f'<input type="hidden" name="redirect_uri" value="https://scanner.invalid/cb">'
    with patch.object(get_registry(), "find_for_host", return_value=_program()):
        with patch(f"{_MODULE}.fetch_for_discovery", return_value=_oidc_outcome()):
            with patch(f"{_MODULE}.submit_probe", return_value=candidate_resp):
                run(scan_run, target_run)
    assert Finding.objects.filter(scan_run=scan_run, stub_slug="2.14",
                                   confidence="medium").exists()
