"""Gate tests for stub 2.4 (weak-rate-limiting)."""
...
def test_non_login_form_present_is_skipped() -> None:
    ...
    outcome = FetchOutcome(
        ok=True, status=200, body=reset_only, content_type="text/html",
...
def test_out_of_scope_final_url_returns_silently() -> None:
    ...
    `enforce_scope` raises OutOfScope, runner returns without
