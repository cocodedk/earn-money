...
def run(
    ...
    *, program: Program,
    ...
    attempts = _run_attempt_loop(
        canary=canary, form=login_form, program=program,
    )
    if attempts is None:
        return
    _emit_finding(
...
def _run_attempt_loop(
    *, canary: str, form: AuthForm, program: Program,
) -> int | None:
    ...
    cap (codex P1.3 generalised across multi-probe stubs).
    """
