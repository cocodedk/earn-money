"""Gate tests for stub 2.12 (weak-recovery-codes)."""
...
@pytest.mark.django_db
def test_recovery_codes_endpoint_unreachable() -> None:
    scan_run, target_run = seed_target_run(host="x.example", stub_slug="2.12")
    with patch.object(get_registry(), "find_for_host", return_value=_program()), \
         patch(f"{_TARGET}.register_via_api",
               return_value=MagicMock(status_code=201)), \
         ...
         patch(f"{_TARGET}.enroll_mfa",
               return_value=MagicMock(status_code=200)), \
         patch(f"{_TARGET}.generate_recovery_codes", return_value=None):
        run(scan_run, target_run)
    ev = Event.objects.get(scan_run=scan_run, type=EventType.AUTH_FIXTURE_REQUIRED)
    assert ev.data["detail"] == "recovery_codes_endpoint_unreachable_or_rejected"


@pytest.mark.django_db
def test_recovery_codes_empty_response() -> None:
    """Endpoint returns 2xx but no codes in the body → refuse."""
    scan_run, target_run = seed_target_run(host="x.example", stub_slug="2.12")
    with patch.object(get_registry(), "find_for_host", return_value=_program()), \
         patch(f"{_TARGET}.register_via_api",
               return_value=MagicMock(status_code=201)), \
         ...
         patch(f"{_TARGET}.generate_recovery_codes",
               return_value=MagicMock(status_code=200)), \
         patch(f"{_TARGET}.extract_codes_from", return_value=[]):
        run(scan_run, target_run)
    ev = Event.objects.get(scan_run=scan_run, type=EventType.AUTH_FIXTURE_REQUIRED)
    assert ev.data["detail"] == "recovery_codes_response_yielded_no_codes"
