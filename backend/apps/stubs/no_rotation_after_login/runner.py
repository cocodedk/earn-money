"""Stub 3.6 runner — no session rotation after login."""
...
from .classify import RotationStatus, classify_rotation

_STUB_ID = "3.6"
_LOGIN_PATH = "/rotation/login-no-rotation"
...
def run(scan_run: ScanRun, target_run: ScanTargetRun) -> None:
    ...
    result = classify_rotation(pre_cookies, post_cookies)
    if result.status is RotationStatus.CONFIRMED:
...
def _emit_finding(
    *, scan_run: ScanRun, target_run: ScanTargetRun,
    ...
    finding = Finding.objects.create(
        ...
        stub_slug=_STUB_ID,
        title="Session ID not rotated after login",
        category="no_rotation_after_login",
