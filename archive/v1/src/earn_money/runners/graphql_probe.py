...
For each in-scope HTTP service URL, hand the URL to `graphql_tool.probe`
which POSTs a minimal introspection query at common GraphQL paths and
emits Signals where `__schema` is publicly enumerable.

Same gate/RoE/manifest pattern as the other Signal-emitting runners.
Independent of nuclei but shares the httpx prereq.
"""
...
def run_program(
    ...
    *,
    ...
    """Gate → prereq → load services → probe → scope-filter → record."""
    ...
    artifact_dir = paths.root / (
        f"recon/outputs/{platform}/{slug}/graphql/{now[:10]}/{run_id}"
    )
    ...
    try:
        if not _recent_httpx_success(conn, platform=platform, slug=slug, now=now_dt):
            return record_prereq_missing(
                ...
                run_id=run_id, now=now, artifact_dir=artifact_dir,
                tool="graphql-probe",
        ...
        runs.start_run(
            ...
            tool="graphql-probe", started_at=now,
        ...
        nuclei_artifacts.write_manifest(artifact_dir, {
            "run_id": run_id, "tool": "graphql-probe",
