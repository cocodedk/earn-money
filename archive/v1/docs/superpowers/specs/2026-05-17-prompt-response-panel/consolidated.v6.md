...
**Supersedes:** `consolidated.v5.md`

## Changes from v5 (simplify pass only — no design changes)

Dedup + cruft removal across the source files. Net source: -10 lines.

- textContent / no-innerHTML rule: canonical statement in `02-sse-event-shape.md § Security` (with the full forbidden-sinks list); `03-right-panel-ux.md § Security` collapsed to a 1-line cross-ref; `06-non-goals.md` test filename corrected.
- `01-architecture.md § Boundary discipline`: 5 detailed bullets that duplicated the 04-file-structure table replaced with 3 cross-module invariants.
- "Loading rule" / "Both the timeline summary..." paragraphs in 01 dropped (covered by 04's load-order block).
- Reset API restated as a single paragraph.
- Stale revision-round commentary removed: "This is the blocker fix", "This is the test the reviewer asked for", "in v1 of this spec", "the reviewer flagged".
- Wordy backcompat note about `TestHooks` lambda signatures tightened.

No acceptance criteria changed. No tests added or removed. No file lists changed.
