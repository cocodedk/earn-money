"""Tests for the graphql-probe runner."""
...
def _signal(host: str) -> Signal:
    return Signal(
        run_id="r", tool="graphql-probe", signal_type="introspection_enabled",
        asset=host, target=f"https://{host}/graphql",
        signature="graphql|/graphql|introspection",
...
def test_manifest_records_roe_block(tmp_repo: Path) -> None:
    ...
    manifest = json.loads(
        next(
            (paths.root / "recon/outputs/hackerone/example/graphql").rglob("manifest.json")
        ).read_text(encoding="utf-8")
    )
    assert manifest["tool"] == "graphql-probe"
...
def test_records_recon_run_row(tmp_repo: Path) -> None:
    ...
    rows = conn.execute(
        "SELECT tool, status FROM recon_runs WHERE tool = 'graphql-probe'"
    ).fetchall()
    conn.close()
    assert rows == [("graphql-probe", "success")]
